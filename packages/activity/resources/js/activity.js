...
jQuery(document).ready(function() {
	...
	activityTable = new ActivityTable({
        //name: tableNames[i],
		name: KD.utils.Util.getParameter("status"),
        ...
        configurationCallback: function(self, options) {
            options.aoColumns = [
				...
				{mData: "Status", sTitle: "Status (Reason)",
					fnRender: function(o){
						if (o.aData["Status Reason"] != null && o.aData["Status Reason"].length>0) {
							return (o.aData["Status"] + " (" + o.aData["Status Reason"] + ")");
						} else {
							return o.aData["Status"];
						}
					},
				...
				//{mData: "Expected Resolution Date", sTitle: "Target Resolution",
				//	fnRender: function(o) {
        ...
        loadCompleteCallback: function(self) {
            ...
            if (self.status === "success") {
				...
				jQuery(".tableControls .tableControl.sortOrder select").val(self.sortOrder);
                jQuery(".tableControls .tableControl.pageSize select").val(self.pageSize);
				jQuery('.tableControls .tableControl.searchValue input[type="text"]').val(self.searchValue);
                ...
                //jQuery(tableContainer).find(".messages .message.success .content").text((offset+1) + "-" + (offset+count) + " " + "of" + " " + total + " " + "records");
	...
	jQuery('.tableControls .tableControl.searchValue input[type="button"]').click(function() {
		activityTable.searchValue = jQuery(this).parents(".tableControl.searchValue").find('input[name="searchValue"]').val();
		activityTable.initialize();
	});
...
function idCellCallback(element, sData, oData, iRow, iColumn) {
	...
	var title='title="' + 'View request fulfillment details' + '"';
	var anchorElement = jQuery('<a ' + title + ' href="javascript:void(0)">'+oData["Id"]+'</a>');
    if (activityTable.name==="Draft") {
		title='title="' + 'Complete current In Progress Request' + '"';
		anchorElement = jQuery('<a ' + title + ' href="DisplayPage?csrv='+oData["Instance Id"]+'&return=yes">'+oData["Id"]+'</a>');
	} else if (activityTable.name==="PendingApproval") {
		title='title="' + 'Complete approval response' + '"';
		anchorElement = jQuery('<a ' + title + ' href="DisplayPage?csrv='+oData["Instance Id"]+'">'+oData["Id"]+'</a>');
	} else if (activityTable.name==="ClosedApproval") {
		title='title="' + 'Review approval response' + '"';
